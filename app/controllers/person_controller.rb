class PersonController < ApplicationController
  ...
  def get_sync_status
    ...
    tasks.each do |task|
      task['target'] = task['target'].gsub(/localhost|172\.0\.0\.1/, '0.0.0.0')
      task['source'] = task['source'].gsub(/localhost|172\.0\.0\.1/, '0.0.0.0')
 ...
 def search_similar_record
  ...
  def get_names
    ...
    if params["last_name"]
      ...
      if data.present?
        render text: data.order('last_name').limit(10).collect(&:last_name).uniq.join("\n") and return
      else
      ...
      if data.present?
        render text: data.order(' first_name ').limit(10).collect(&:first_name).sort.uniq.join("\n") and return
      else
        render text: "" and return
      end
    end
